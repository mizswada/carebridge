...
if (detail.value.response === 200)
{
  ...
  const clickApprove = async () => {        
    try {
        const { data:approved} = await useFetch("/api/rehab-center/pending/approved", {
            method: "POST",
            body: JSON.stringify({                  
              id: id,
            }),
        ...
        if (approved.value.response === 200) 
        {
          ...
          setTimeout(() => {
            $router.push("/rehab-center/pending");
          }, 1000);
        } 
        else 
        {
  ...
  const clickReject = async () => {        
    try {
        const { data:rejected} = await useFetch("/api/rehab-center/pending/rejected", {
            method: "POST",
            body: JSON.stringify({                  
              id: id,
            }),
    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-1">
          <nuxt-link :to="`/rehab-center/pending`">
            <rs-button >
              <Icon
