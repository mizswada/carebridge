<script setup>  
  ...
  const fetchJobs = async () => {
    try {
        jobRespond.value = await useFetch("/api/caretaker/jobs/list", {
            method: "GET",
            query: {
                id: parseInt(userId),
            },
    ...
    <div class="space-y-8">
      ...
      <div class="pb-20">
        <div class="bg-white p-6 rounded-lg shadow-md">
          ...
          >
          <template #title ="jobs" >
                {{ jobs.value.jobs.job_title}}
          </template>
          <template #category ="jobs" >
                {{ jobs.value.jobs.category.name}}
          </template>
          <template #state ="jobs" >
                {{ jobs.value.jobs.lookup_jobs_job_location_stateTolookup.lookupValue}}
          </template>
          <template #date ="jobs" >
                {{ jobs.value.jobs.job_date}}
          </template>
          <template #status ="jobs" >
                {{ jobs.value.lookup_jobs_user_assignation_jobUser_jobStatusTolookup.lookupValue}}
          </template>
          <template #createdAt ="jobs" >
                {{ formatDate(jobs.value.created_at)}}
          </template>
          <template #amount ="jobs" >
                RM {{jobs.value.jobs.job_payment}}
          </template>
          </rs-table>
