<script setup>
    ...
    // list
    const equipment = await useFetch("/api/customer/equipment/list", {
        method: "GET",
        query: {
            id: parseInt(id),
        },
    ...
    const clickAdd = async () => 
    {
        ...
        try {
          ...
          const { data:uploadImage } = await useFetch("/api/customer/equipment/upload", {
            method: 'POST',
            body: {
          ...
          {         
            // alert('masuk');  
            const { data: add } = await useFetch("/api/customer/equipment/create", {
                method: "POST",
                body: JSON.stringify({  
                    user_id : id,
    ...
    {       
      selectedEquipment.value=id;
        try {
            const { data: detail } = await useFetch("/api/customer/equipment/get", {
                method: "GET",
                query: {
                    id: parseInt(id),
                },
    ...
    const clickUpdate = async () => 
    {
      if (!validateFields()) {
        ...
        try {
            const { data: update } = await useFetch("/api/customer/equipment/update", {
                method: "POST",
                body: JSON.stringify({  
    ...
    {
      try {
        const { data:deleted} = await useFetch("/api/customer/equipment/delete", {
            method: "POST",
            body: JSON.stringify({                  
