<script setup>
    ...
    const clickAdd = async () => {
        ...
        try {
          ...
          if(uploadImage.value.respond == 200) 
          {
            ...
            const { data: add } = await useFetch("/api/association/activity/create", {
                method: "POST",
                body: JSON.stringify({  
                    user_id : id,
    ...
    const clickUpdate = async () => 
    {
      ...
      // alert(imageInputEdit.value);
      try {
        const { data: update } = await useFetch("/api/association/activity/update", {
            method: "POST",
            body: JSON.stringify({  
    ...
    {
      try {
        const { data:deleted} = await useFetch("/api/association/activity/delete", {
            method: "POST",
            body: JSON.stringify({                  
