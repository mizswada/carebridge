<script setup>
    ...
    // list
    const rehab = await useFetch('/api/association/list');
    if(rehab.data.value.response == 200)
    {
      association.value = rehab.data.value.data;
    }
    else
    {
    ...
    {
      try {
        const { data:deleted} = await useFetch("/api/association/delete", {
            method: "POST",
            body: JSON.stringify({                  
    ...
    <div class="space-y-8">
        ...
        <rs-card>
          <div class="summary-2 pt-5 pb-3 px-5 flex items-center gap-4">
            ...
            <div class="flex-1 truncate">
              <span class="block font-semibold text-xl leading-tight">{{ association.length }}</span>
      ...
      <div class="pb-20">
        <div class="bg-white p-6 rounded-lg shadow-md">
          <div class="text-lg font-medium mb-4">List of Association</div>
          <div class="flex justify-end items-center mb-3 gap-5">
            <nuxt-link :to="`/association/list/create`">
                <rs-button variant="primary">
                    <Icon name="solar:add-square-broken" class="mr-2" /> Add Association             
                </rs-button>
            </nuxt-link>
            <!-- <rs-button variant="primary" @click="showModal2 = true">            
              <Icon name="solar:add-square-broken" class="mr-2" /> Add Association
            </rs-button>
            <rs-modal title="Add Association"  v-model="showModal2"  size="lg" cancel-title="Close" ok-title="Save">
            ...
            <template #name ="association" >
                {{ association.value.user.username}} <br>
                {{ association.value.user.fullName}}
            </template>
            <template #contact ="association" >
                {{ association.value.user.email}}<br>
                {{ association.value.user.phone}}
            </template>
            <template #address ="association" >
                  {{ association.value.address.line1}}
                  {{ association.value.address.line2}}
                  {{ association.value.address.city}}<br>
                  {{ association.value.address.postcode}}
                  {{ association.value.address.state}}
                  {{ association.value.address.country}}
            </template>
            <template #status ="association" >
                  {{ association.value.user.status}}
            </template>
            <template #action ="association" >
              <div class="flex items-center gap-4">
                <nuxt-link :to="`/association/list/view/`+association.value.id">
                  <rs-button >
                    <Icon
                ...
                <!-- <nuxt-link :to="`/association/list/activity/`+association.value.id">
                    <rs-button variant="info">
                        <Icon name="material-symbols:list-alt-outline-rounded" class="mr-2 !w-4 !h-4" /> Activity             
                    </rs-button>
                </nuxt-link>

                <nuxt-link :to="`/association/list/donation/`+association.value.id">
                    <rs-button variant="success">
                        <Icon name="solar:hand-money-outline" class="mr-2 !w-4 !h-4" /> Donation             
                    </rs-button>
                </nuxt-link> -->
                
                <!-- <rs-button variant="danger" @click="confirmDelete(association.value.id)">
                  <Icon
