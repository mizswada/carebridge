<script setup>
    ...
    const clickAdd = async () => 
    {
        ...
        try {
          ...
          const { data:uploadImage } = await useFetch("/api/association/equipment/upload", {
            method: 'POST',
            body: {
          ...
          {         
            // alert('masuk');  
            const { data: add } = await useFetch("/api/association/equipment/create", {
                method: "POST",
                body: JSON.stringify({  
                    user_id : id,
    ...
    {       
      selectedEquipment.value=id;
        try {
            const { data: detail } = await useFetch("/api/association/equipment/get", {
                method: "GET",
                query: {
                    id: parseInt(id),
                },
    ...
    const clickUpdate = async () => 
    {
        ...
        try {
            const { data: update } = await useFetch("/api/association/equipment/update", {
                method: "POST",
                body: JSON.stringify({  
    ...
    {
      try {
        const { data:deleted} = await useFetch("/api/association/equipment/delete", {
            method: "POST",
            body: JSON.stringify({                  
