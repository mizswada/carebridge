...
const { data: paymentInfo } = await useFetch(
  "/api/apps/care-services/payment/receipt/get",
  {
    method: "GET",
    query: {
      orderID: orderID,
  ...
  <div class="flex flex-col items-center p-6 space-y-4 bg-gray-50 min-h-screen">
    ...
    <div class="w-full max-w-sm p-6 bg-white rounded-xl shadow-lg">
      <div v-if="statusID == 1" class="flex flex-col items-center space-y-2">
        ...
        <p class="text-2xl font-bold text-gray-800">RM {{ paymentInfo.data.paymentDetails.job_payment }}</p>
      </div>
    ...
    <div class="w-full max-w-sm p-6 bg-white rounded-xl shadow-lg">
      ...
      <div class="mt-4 space-y-3 text-sm text-gray-600">
        <div class="flex justify-between">
          ...
          <span class="font-medium">{{ paymentInfo.data.paymentDetails.job_paymentReferenceNum }}</span>
        </div>
        ...
        <div class="flex justify-between">
          ...
          <span class="font-medium">{{ DateTime.fromISO(paymentInfo.data.paymentDetails.job_paymentDate).toFormat("yyyy-MM-dd hh:mm:ss") }} </span>
        </div>
        <div class="flex justify-between">
          ...
          <span class="font-medium">RM
                {{
                    ...
                    paymentInfo.data.paymentDetails.job_payment
