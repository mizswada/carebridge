...
export default defineEventHandler(async (event) => {
  try {
    ...
    const dir = path.join(process.cwd(), 'public/uploads/caretaker/profile');
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }

    const uniqueFilename = "certificate_"+Date.now() + "_" + Math.floor(Math.random() * 1000);
    const fileName = uniqueFilename + "_"+ body.fileName;
    // Specify the path where you want to save the file
    const fileUpload = path.join(process.cwd(), `public/uploads/caretaker/profile/${fileName}`);
    //const fileUpload = `/home/ascii/ALSB/public/img/uploads/${body.operationID}/${fileName}`;
    const filePath = `/uploads/caretaker/profile/${fileName}`;
